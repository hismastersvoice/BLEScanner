...
void OTAWebServer::postUpdateConfig()
{
  ...
  std::map<String, ParamHandler> paramHandlers = {
      ...
      {"mqtttimeout", [&](const String &val)
       { newSettings.mqttTimeout = val; }},
      {"udpsrvr", [&](const String &val)
       { newSettings.udpServer = val; }},
      {"udpport", [&](const String &val)
       { newSettings.udpPort = atoi(val.c_str()); }},
      {"dhcpenabled", [&](const String &val)
       { newSettings.EnableDHCP(val == "true"); }},
      {"mqttenabled", [&](const String &val)
       { newSettings.EnableMQTT(val == "true"); }},
      {"udpenabled", [&](const String &val)
       { newSettings.EnableUDP(val == "true"); }},
      {"haenabled", [&](const String &val)
       { newSettings.EnableHA(val == "true"); }},
      {"devenabled", [&](const String &val)
       { newSettings.EnableDEV(val == "true"); }},
      {"netip", [&](const String &val)
       { newSettings.netIp = val; }},
      {"netsub", [&](const String &val)
       { newSettings.netSub = val; }},
      {"netgate", [&](const String &val)
       { newSettings.netGate = val; }},
      {"netdns", [&](const String &val)
       { newSettings.netDns = val; }},
      {"netdns2", [&](const String &val)
       { newSettings.netDns2 = val; }},
      {"location", [&](const String &val)
       { newSettings.location = val; }},
...
void OTAWebServer::sendSysInfoData(bool trackerInfo, bool deviceList)
{
  ...
  if (trackerInfo)
  {
    ...
    if (SettingsMngr.devEnabled)
    {
      ...
      SendChunkedContent(R"(",)");
      SendChunkedContent(R"("sdk_version":")");
      SendChunkedContent(ESP.getSdkVersion());
      SendChunkedContent(R"(",)");      
      char chip_freq[10];
      char chip_core[10];
      char chip_rev[10];
      ...
      itoa(ESP.getChipRevision(), chip_rev, 10);
      float esp_temp = 0;
      temp_sensor_read_celsius(&esp_temp);
      SendChunkedContent(R"("chip":")");
      SendChunkedContent(ESP.getChipModel());
      SendChunkedContent(R"(",)");
      SendChunkedContent(R"("chip_revision":")");
      SendChunkedContent(chip_rev);
      SendChunkedContent(R"(",)");
      SendChunkedContent(R"("chip_freq":")");
      SendChunkedContent(chip_freq);
      SendChunkedContent(R"(",)");
      SendChunkedContent(R"("chip_core":")");
      SendChunkedContent(chip_core);
      SendChunkedContent(R"(",)");
      ...
      itoa(ESP.getHeapSize() - ESP.getFreeHeap(), heap_used, 10);
      SendChunkedContent(R"("heap_free":")");
      SendChunkedContent(heap_free);
      SendChunkedContent(R"(",)");
      SendChunkedContent(R"("heap_total":")");
      SendChunkedContent(heap_size);
      SendChunkedContent(R"(",)");
      SendChunkedContent(R"("heap_min":")");
      SendChunkedContent(heap_min);
      SendChunkedContent(R"(",)");
      SendChunkedContent(R"("heap_used":")");
      SendChunkedContent(heap_used);
      SendChunkedContent(R"(",)");
      char psram_total[10];
      char psram_used[10];
      itoa(ESP.getPsramSize(), psram_total, 10);
      itoa(ESP.getPsramSize() - ESP.getFreePsram(), psram_used, 10);
      SendChunkedContent(R"("ps_ram":")");
      SendChunkedContent(psram_total);
      SendChunkedContent(R"(",)");
      SendChunkedContent(R"("ps_ram_used":")");
      SendChunkedContent(psram_used);
      SendChunkedContent(R"(",)");
      char sketch_total[10];
      char sketch_used[10];
      itoa(ESP.getFreeSketchSpace(), sketch_total, 10);
      itoa(ESP.getSketchSize(), sketch_used, 10);
      SendChunkedContent(R"("sketch_total":")");
      SendChunkedContent(sketch_total);
      SendChunkedContent(R"(",)");
      SendChunkedContent(R"("sketch_used":")");
      SendChunkedContent(sketch_used);
      SendChunkedContent(R"(",)");
      char flashsize[10];
      itoa(ESP.getFlashChipSize(), flashsize, 10);
      SendChunkedContent(R"("flashsize":")");
      SendChunkedContent(flashsize);
      SendChunkedContent(R"(",)");
    }
