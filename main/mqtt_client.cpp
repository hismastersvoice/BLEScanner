...
namespace MQTTClient
{
  ...
  */
  ...
  const char *getMQTTBaseSensorTopic()
  {
    ...
    if (MQTT_BASE_SENSOR_TOPIC[0] == 0)
    {
      ...
      strncpy(location, SettingsMngr.location.c_str(), sizeof(location));
  ...
  */
  ...
  {
    const unsigned long AVAILABILITY_UPDATE_INTERVAL = 120; // 2 minutes
  ...
  void publishAvailabilityToMQTT()
  {
    if (NTPTime::seconds() > lastMQTTConnection)
    {
     ...
     // ENABLE_HOME_ASSISTANT_MQTT_DISCOVERY
    }
  }
