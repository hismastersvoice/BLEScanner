...
namespace WiFiManager
{
  ...
  const char *GetDefaultHostName()
  {
    ...
    WiFi.macAddress(mac);
    static char ssid[32];
    snprintf(ssid, sizeof(ssid), "ESP32-%02X%02X%02X%02X%02X%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
    return ssid;
  }

  const char *GetDefaultGatewayName()
  {
    return GATEWAY_NAME[0] == '\0' ? GetDefaultHostName() : GATEWAY_NAME;
  }

  static std::vector<std::function<void(void)>> WiFiConnectedCallbacks;
  void AddWiFiConnectedCallback(std::function<void(void)> callback)
  {
    WiFiConnectedCallbacks.push_back(callback);
  }
  ...
  void StartAccessPointMode()
  {
    ...
    WiFi.softAP(GetDefaultHostName());
  ...
  bool WiFiConnect(const String &_ssid_, const String &_password_)
  {
    if (WiFi.status() != WL_CONNECTED && !_ssid_.isEmpty())
    {
      ...
      if (!SettingsMngr.dhcpEnabled)
      {
        IPAddress ip, netmask, gateway, primaryDNS, secondaryDNS;
      ip.fromString(SettingsMngr.netIp);
      netmask.fromString(SettingsMngr.netSub);
      gateway.fromString(SettingsMngr.netGate);
      primaryDNS.fromString(SettingsMngr.netDns);
      secondaryDNS.fromString(SettingsMngr.netDns2);
      if (!WiFi.config(ip, gateway, netmask, primaryDNS, secondaryDNS))
      {
        const char *errMsg = "Error: WiFi configuration failed";
    ...
    for (auto &callback : WiFiConnectedCallbacks)
    {
      callback();
    }
  }
